...
COMMANDS = (
    ...
    Command("check", "", ("Summarize Codex Doctor diagnostics as JSON.", "以 JSON 汇总 Codex Doctor 诊断。"),
            ...
            ("codex-socks check",)),
    Command("scope", "--client {codex,claude} [--entry {direct,managed}] [--target URL] [--json]",
            ("Show proxy scope and transport coverage.", "显示代理作用域和传输覆盖。"),
            ("Read-only. Compares the current process environment with a managed launch plan and labels HTTP, WebSocket, remote MCP, stdio MCP and official connectors separately. It does not test the network.",
             "只读。对比当前进程环境和受管启动计划，分别标记 HTTP、WebSocket、远程 MCP、stdio MCP 和官方连接器；不会联网测试。"),
            ("codex-socks scope --client claude", "codex-socks scope --client codex --target https://api.example.test --json")),
    Command("policy", "set --client {codex,claude} --mode {inherit,profile,off} [--profile NAME]",
            ("Set a client's managed launch policy.", "设置客户端的受管启动策略。"),
            ("Claude defaults to inherit. Codex keeps its legacy active profile and supports profile or off. Saving affects new managed launches only.",
             "Claude 默认继承。Codex 保留原有活动配置，仅支持 profile 或 off。保存后只影响新的受管启动。"),
            ("codex-socks policy set --client claude --mode profile --profile office",
             "codex-socks policy set --client claude --mode inherit")),
    Command("bypass", "{add,remove,list} --client {codex,claude} [HOST]",
            ("Manage per-client NO_PROXY rules.", "管理每个客户端的 NO_PROXY 规则。"),
            ("Rules are hosts or IPs, not URLs. Client rules are added to inherited bypass values for managed launches.",
             "规则必须是主机或 IP，不能是 URL。受管启动会把客户端规则加入继承的绕过值。"),
            ("codex-socks bypass add --client claude api.deepseek.example",
             "codex-socks bypass list --client claude")),
    Command("run", "--client {codex,claude} [--profile NAME | --inherit | --off] [--] [ARGS...]",
            ("Start a client with an explicit proxy scope.", "使用明确的代理作用域启动客户端。"),
            ("Replaces the current process. One-time mode options override the saved policy. Claude rejects SOCKS profiles before launch; existing sessions are unchanged.",
             "替换当前进程。一次性模式参数覆盖已保存策略。Claude 在启动前拒绝 SOCKS 配置；已有会话不变。"),
            ("codex-socks run --client claude", "codex-socks run --client claude --profile office -- --debug")),
